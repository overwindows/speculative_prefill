...
def _forward_with_query_dump(
    ...
    if q.shape[-2] > 1:
        ...
        query_buffer[layer_idx].append(q[-1:, :])
    else:
...
class LookAheadSpecWorker(Worker):
    def load_model(self):
        ...
        num_heads = getattr(cfg, "num_attention_heads",
                            None) or getattr(cfg, "n_head", None)
        num_kv_heads = getattr(cfg, "num_key_value_heads", None) or getattr(
            cfg, "num_kv_heads", None) or num_heads
        head_dim = getattr(cfg, "head_dim", None) or getattr(
            cfg, "hidden_size", None) // num_heads if num_heads and getattr(cfg, "hidden_size", None) else None
        if not all([num_heads, num_kv_heads, head_dim]):
            raise RuntimeError(
                "Could not infer (num_heads, num_kv_heads, head_dim) for model config.")
    ...
    def _get_kept_indices_from_token_importance(
        ...
        for sample_ti in token_importance:
            ...
            if self.spec_config.keep_kwargs.get("chunk", False):
                ...
                indices = torch.split(torch.arange(
                    seq_len), chunk_size, dim=-1)
                indices = torch.concat([indices[ci.item()]
    ...
    def _get_attention_scores(
        ...
        for sample_idx in range(len(attn_weights[0])):
            reshaped_attn_weights.append(
                torch.stack(
                    [attn_weights[layer_idx][sample_idx]
